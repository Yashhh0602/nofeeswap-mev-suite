...
import { parseAbi, encodeAbiParameters, keccak256, encodePacked } from 'viem'
...
// Mock kernel from SwapData_test.py#L841-846
  ...
  return (
    <div>
      <div className="flex justify-between items-center mb-2">
        <span className="text-xs text-gray-400">Kernel Shape (click to add points, right-click to remove)</span>
...
export default function PoolPage() {
  ...
  const [token0, setToken0] = useState(CONTRACTS.token0)
  const [token1, setToken1] = useState(CONTRACTS.token1)
  ...
  const poolId = (() => {
    try { return computePoolId(token0, token1, parseInt(fee)) } catch { return null }
  })()
  ...
  const handleInit = () => {
    ...
    writeContract({
      ...
      args: [poolId, sqrtPriceX96, MOCK_KERNEL, '0x'],
    })
  }
      ...
      <div className="max-w-3xl mx-auto">
        ...
        <div className="grid grid-cols-2 gap-6">
          <div className="space-y-4">
            <div>
              <label className="text-xs text-gray-400 block mb-1">Token 0</label>
              <input value={token0} onChange={e => setToken0(e.target.value)}
                className="w-full bg-gray-900 border border-gray-700 rounded px-3 py-2 text-sm font-mono focus:border-green-400 outline-none" />
            </div>
            <div>
              <label className="text-xs text-gray-400 block mb-1">Token 1</label>
            ...
            {isSuccess && (
              <div className="bg-green-900 border border-green-500 rounded p-3 text-sm text-green-200">
                ✓ Pool initialized! Tx: {hash?.slice(0,18)}...
              </div>
            )}
            {error && (
            ...
            <div className="bg-gray-900 border border-gray-800 rounded p-4">
              ...
              {useMock ? (
                <div className="text-center py-8 border border-dashed border-gray-700 rounded">
                  ...
                  <p className="text-green-400 text-xs mt-1">[(2^15, 2^15), (2^15, 0)]</p>
                </div>
              ) : (
